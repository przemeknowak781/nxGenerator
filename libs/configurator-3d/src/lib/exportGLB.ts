...
export interface ExportOptions {
  /** Arbitrary configuration to embed under `asset.extras[metaKey]`. */
  metadata?: unknown;
  /** Key used for the embedded metadata. Defaults to `configuratorConfig`. */
  metaKey?: string;
  /** App works in millimetres; `'meters'` (default) scales the export by 0.001. */
  unitScale?: 'meters' | 'mm';
  /** Version string stored alongside the metadata. */
  version?: string;
}

/**
 * Export a Three.js object tree to a binary glTF (GLB) blob, optionally
 * embedding a round-trippable configuration snapshot in `asset.extras`.
 *
 * This is the shared export engine for every configurator: the domain only
 * supplies the object to export and its config; the GLB plumbing lives here.
 */
export async function exportSceneToGLB(
  ...
  const key = opts.metaKey ?? 'configuratorConfig';
  const finalBuf = injectExtras(result, {
    [key]: opts.metadata,
...
/** Splice extra keys into the GLB's JSON chunk `asset.extras`, rebuilding the container. */

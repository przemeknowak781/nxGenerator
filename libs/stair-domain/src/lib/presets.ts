...
export const PRESET_LIST: StairPreset[] = [
  {
...
export const PRESETS: Record<string, StairPreset> = Object.fromEntries(
  PRESET_LIST.map((p) => [p.id, p]),
);

/** Lightweight list for preset pickers (id + label only). */
export const PRESET_REFS: PresetRef[] = PRESET_LIST.map(({ id, label }) => ({ id, label }));

/** Apply a stair preset by id, deep-merging its per-element material patch. */

import { runRules, type Rule, type Issue } from '@nxgen/configurator-core';
import type { StairConfig, BuildingType } from './types';
import { computeMetrics } from './metrics';

/**
 * Live compliance validator for the Polish building code (Warunki Techniczne
 * §68) with three building profiles. Each rule is a {@link Rule}<StairConfig>;
 * the actual pass/collect is handled by the shared engine's {@link runRules}.
 */
...
type StairRule = (c: StairConfig, t: Thresholds) => Issue | null;

const ruleRiseMax: StairRule = (c, t) => {
...
const ruleWalkMin: StairRule = (c, t) => {
...
const ruleWidthMin: StairRule = (c, t) => {
...
const ruleRailHeight: StairRule = (c, t) => {
...
const ruleBarSpacing: StairRule = (c, t) => {
...
const ruleStepAngle: StairRule = (c, t) => {
...
const STAIR_RULES: StairRule[] = [
  ruleRiseMax, ruleWalkMin, ruleWidthMin,
  ruleRailHeight, ruleBarSpacing,
  ruleBlondel, ruleStepAngle,
];

/** Build the config-bound rule list for the active building type. */
export function stairRules(buildingType: BuildingType): Rule<StairConfig>[] {
  const t = T[buildingType];
  return STAIR_RULES.map((rule) => (c: StairConfig) => rule(c, t));
}

/** Validate a stair configuration against WT §68 for its building type. */
export function validate(c: StairConfig): Issue[] {
  return runRules(c, stairRules(c.buildingType));
}
